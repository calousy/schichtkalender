...
import com.kmeisl.schichtkalender.domain.Schicht;
...
public final class OffeneSchichtDetailsWindow extends Window {

	private final Label synopsis = new Label();

	private OffeneSchichtDetailsWindow(final OffeneSchicht schicht, final Date startTime, final Date endTime) {
		addStyleName("moviedetailswindow");
		Responsive.makeResponsive(this);

		setCaption(schicht.getTitle());
		center();
		...
		Panel detailsWrapper = new Panel(buildMovieDetails(schicht, startTime, endTime));
	...
	private Component buildMovieDetails(final OffeneSchicht movie, final Date startTime, final Date endTime) {
...
//		details.addComponent(coverImage);
	...
	private Component buildDetailsForm(final OffeneSchicht movie, final Date startTime, final Date endTime) {
...
//		fields.addComponent(synopsis);
		...
		more.addClickListener(new ClickListener() {
			...
			public void buttonClick(final ClickEvent event) {
				...
				OffeneSchichtDetailsWindow.this.focus();
			}
		});
	...
	private void updateSynopsis(final Schicht m, final boolean expand) {
...
//			synopsis.setData(m.getSynopsis());
	...
	public static void open(final OffeneSchicht movie, final Date startTime, final Date endTime) {
		SchichtkalenderEventBus.post(new CloseOpenWindowsEvent());
		Window w = new OffeneSchichtDetailsWindow(movie, startTime, endTime);
