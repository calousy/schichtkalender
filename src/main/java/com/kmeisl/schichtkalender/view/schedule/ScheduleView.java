package com.kmeisl.schichtkalender.view.schedule;
...
import com.kmeisl.schichtkalender.component.OffeneSchichtDetailsWindow;
import com.kmeisl.schichtkalender.domain.OffeneSchicht;
...
public final class ScheduleView extends CssLayout implements View {
	...
	public void detach() {
		...
		SchichtkalenderEventBus.unregister(this);
	}
	...
	private void injectMovieCoverStyles() {
		...
		String styles = "";
		// for (OffeneSchicht m :
		// SchichtkalenderUI.getDataProvider().getMovies()) {
	...
	private Component buildCalendarView() {
		...
		calendar.setHandler(new EventClickHandler() {
			...
			public void eventClick(final EventClick event) {
				...
				OffeneSchichtEvent movieEvent = (OffeneSchichtEvent) event.getCalendarEvent();
				OffeneSchichtDetailsWindow.open(movieEvent.getMovie(), movieEvent.getStart(), movieEvent.getEnd());
			}
		});
		...
		calendar.setHandler(new BasicEventMoveHandler() {
			...
			public void eventMove(final MoveEvent event) {
				...
				if (calendarEvent instanceof OffeneSchichtEvent) {
					OffeneSchichtEvent editableEvent = (OffeneSchichtEvent) calendarEvent;
			...
			protected void setDates(final OffeneSchichtEvent event, final Date start, final Date end) {
				event.start = start;
				event.end = end;
			}
		});
		...
		initialView.add(java.util.Calendar.DAY_OF_WEEK, -initialView.get(java.util.Calendar.DAY_OF_MONTH) + 1);
		initialView.set(initialView.get(java.util.Calendar.YEAR), initialView.get(java.util.Calendar.MONTH), 1);
		calendar.setStartDate(initialView.getTime());

		initialView.set(initialView.get(java.util.Calendar.YEAR), initialView.get(java.util.Calendar.MONTH), initialView.getActualMaximum(java.util.Calendar.DAY_OF_MONTH));
	...
	private class MovieEventProvider implements CalendarEventProvider {
		...
		public List<CalendarEvent> getEvents(final Date startDate, final Date endDate) {
			...
			// when needed.
//			Collection<Transaction> transactions = SchichtkalenderUI.getDataProvider().getTransactionsBetween(startDate, endDate);
			List<CalendarEvent> result = new ArrayList<CalendarEvent>();
//			for (Transaction transaction : transactions) {
//				Movie movie = SchichtkalenderUI.getDataProvider().getMovie(transaction.getMovieId());
...
//			}
	...
	public final class OffeneSchichtEvent implements CalendarEvent {

		private Date start;
		private Date end;
		private OffeneSchicht movie;

		public OffeneSchichtEvent(final Date start, final Date end, final OffeneSchicht movie) {
			this.start = start;
			this.end = end;
			this.movie = movie;
		}
		...
		public String getStyleName() {
			return String.valueOf(movie.getSchichtId());
		}
		...
		public OffeneSchicht getMovie() {
			return movie;
		}

		public void setMovie(final OffeneSchicht movie) {
			this.movie = movie;
		}
